					...
					if ($data->num_rows > 0) {
						while($line = $data->fetch_assoc()){
							...
							if($pass1 == $pass2){
								...
								$colde = $_POST['code'];
								header("Location: /main/passrecovery.php?email=$email2&code=$colde&id=2");
							}
						}
					}
					else {
  ...
  <div class="content" align="center" style="background:#F5F5F5;" id="msg"> 
	...
	<?php 
		if($sucess == "true"){
	  	...
	  	elseif($right == "true"){
	  		echo "<form action='/services/recoverpass.php' method='post'><input name='email' id='email' type='text' class='input' placeholder='Insira o email cadastrado no site' required><br><input value='Enviar Mensagem' type='submit' class='bufiles' style='width: 80%;padding: 10px 25px;box-sizing: content-box;max-width:250px;'></label></form>";
		}
	?>
  </div>
<div class="footer">
