<?php 
	$conn = new mysqli('YOUR HOST', 'YOUR USER', 'PSWD', 'DBNAME');
	$sql = "SELECT * FROM map";
  ...
  <div class="divmap" id="map" onmousedown='mydragg.startMoving(this,"container",event);' onmouseup='mydragg.stopMoving("container");' style="top:-2160px;left:-1160px;"> 
    <?php
			foreach($id as $name){
				...
				echo '<div class="spot tooltip" id="c'.$name.'" style="top:'.$top[$name].';left:'.$left[$name].';background:url('."'".'img/map/' . $flag[$name];
				if(!$accurate[$name]){
